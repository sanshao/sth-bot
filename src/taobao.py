...
def process_taobao_file(file_path, target_directory):
    ...
    category_mapping = {
        ...
        "淘宝-基础服务费":["基础软件服务费", "淘宝天猫跨境服务基础费"],
        # "淘宝-交易收款":[""],
        "淘宝-交易退款":["淘宝消费者保证金-交易售后", "售后退款-"],
        "淘宝-保证金退款":["保证金退款"],
        "淘宝-跨境服务费":["淘宝天猫跨境服务增值费"],
        "淘宝-赔付":["淘宝消费者保证金-充值（代扣）-缺货", "支付宝转账小额打款-关联订单号", "淘宝消费者保证金-充值（代扣）-红包冻结", "淘宝消费者保证金-充值（代扣）-延迟发货", "商家权益红包-预算追加-卖家延迟发货赔付红包-赔付红包"],
        "淘宝-品牌护肤赤兔":["代扣-赤兔名"],
    ...
    with pd.ExcelWriter(new_file_path, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='整理1', index=False)
        pivot_table.to_excel(writer, sheet_name='透视1', index=False)
